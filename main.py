...
if initialise.INITIAL_MEMORY_VERSION is None:
    ...
    print('{}{}{}'.format(
        'LOADING MEMORY VERSION ',
        str(initialise.INITIAL_MEMORY_VERSION),
        '...')
    )
    memory = pickle.load(open('{}{}{}{}{}{}{}'.format(
        run_archive_folder,
        env.name,
        '/run',
        str(initialise.INITIAL_RUN_NUMBER).zfill(4),
        "/memory/memory",
        str(initialise.INITIAL_MEMORY_VERSION).zfill(4),
        ".p",
        "rb"
    )))
...
while 1:
    ...
    if len(memory.ltmemory) >= config.MEMORY_SIZE:
        ...
        for s in memory_samp:
            ...
            lg.logger_memory.info(
                'CUR PRED VALUE FOR %s: %f',
                s['playerTurn'],
                current_value
            )
            lg.logger_memory.info(
                'BES PRED VALUE FOR %s: %f',
                s['playerTurn'],
                best_value
            )
            lg.logger_memory.info(
                'THE MCTS ACTION VALUES: %s',
                ['%.2f' % elem for elem in s['AV']]
            )
            lg.logger_memory.info(
                'CUR PRED ACTION VALUES: %s',
                ['%.2f' % elem for elem in  current_probs]
            )
            lg.logger_memory.info(
                'BES PRED ACTION VALUES: %s',
